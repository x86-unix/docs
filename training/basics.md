...
```mermaid
graph LR
    CPU["CPU 頭脳"] <--> Memory["メモリ 一時記憶/高速"]
    CPU <--> HDD["ハードディスク 永続記憶/低速"]
    Memory <-.-> |"頻繁なデータをキャッシュ"| HDD
```
...
```mermaid
graph TB
    App["アプリケーション 自社サービス/Webサイト"] --> MW["ミドルウェア Apache/nginx/MySQL"]
    MW --> OS2["OS Linux/Windows"]
    OS2 --> HW2[ハードウェア]
```
...
graph LR
    subgraph "パケットの構造"
        Header["ヘッダ 宛先/送信元/制御情報"] --> Data["データ 運びたい情報本体"]
    end
```
...
```mermaid
graph LR
    PC1["PC 192.168.0.10"] --> Router["ルーター NAT変換"]
    PC2["PC 192.168.0.11"] --> Router
    Router -->|"グローバルIP 203.0.113.1"| Internet[インターネット]
```
...
```mermaid
graph LR
    Client[クライアント] --> LB["ロードバランサ ALB/NLB"]
    LB --> S1[サーバー1]
    LB --> S2[サーバー2]
    LB --> S3[サーバー3]
```
